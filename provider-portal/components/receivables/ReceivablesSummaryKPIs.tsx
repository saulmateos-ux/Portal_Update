  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      {kpis.map((kpi, index) => (
        <div
          key={index}
          className={`bg-white rounded-lg border ${kpi.highlight ? 'border-semantic-success shadow-md' : 'border-brand-neutral-40'} p-6 hover:shadow-lg transition-shadow`}
        >
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1">
              <p className="text-sm font-medium text-brand-neutral-80">{kpi.title}</p>
            </div>
          ...
          <div>
            ...
            <p className="text-xs text-brand-neutral-60 mb-2">{kpi.subtitle}</p>
            {kpi.description && (
              <p className="text-xs text-brand-neutral-60 italic">{kpi.description}</p>
            )}
          </div>
        </div>
      ))}
