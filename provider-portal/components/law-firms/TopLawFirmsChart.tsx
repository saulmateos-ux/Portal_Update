...
export function TopLawFirmsChart({ lawFirms, loading }: TopLawFirmsChartProps) {
  ...
  // Color based on collection rate - using design tokens
  const getBarColor = (rate: number) => {
    if (rate >= 80) return COLORS.semantic.success;  // green - excellent
    if (rate >= 60) return COLORS.semantic.info;     // blue - good
    if (rate >= 40) return COLORS.semantic.warning;  // amber - fair
    return COLORS.semantic.danger;                    // red - poor
  };
